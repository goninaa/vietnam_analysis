...
class FileData:
    ...
    def create_df(self) -> None:
        ...
        # df.columns = ["start_time", "end_time", "real_start", "real_end", "activity"]
        df.columns = ["start_time", "end_time","activity"]
        # df['fstart'] = pd.to_timedelta(self.fstart)
        df['start_time'] = pd.to_timedelta(df['start_time'], unit='s')
        df['end_time'] = pd.to_timedelta(df['end_time'], unit='s')
        # df['fname'] = datetime.datetime.strptime(f'{self.hour}:{self.minutes}:{self.seconds}', '%H:%M:%S').time()
        # date_time = datetime.datetime.strptime(df['fname'] '%H:%M:%S.%f') #check
